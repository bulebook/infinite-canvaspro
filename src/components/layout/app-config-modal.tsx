...
import { API_PRESETS } from "@/constant/api-presets";
...
export function AppConfigModal() {
    ...
    const addPresetChannel = (presetId) => {
        const preset = API_PRESETS.find(p => p.id === presetId);
        if (!preset) return;
        updateChannels([...config.channels, createModelChannel({ ...preset.channel, name: preset.channel.name })]);
    };
